...
public class Proceso implements Runnable {
    ...
    public Proceso(int id, String nombre, EstadoProceso estado, int instruccionesRestantes, int prioridad, TipoBound tipo, int ciclosParaInterrupcion, int ciclosParaSatisfacerIO, int programCounter, String registroInstruccion, int registroA, int contadorCiclos, int contadorIOCiclos) {
        this(id, nombre, estado, instruccionesRestantes, prioridad, tipo, ciclosParaInterrupcion, ciclosParaSatisfacerIO, programCounter, registroInstruccion, registroA, contadorCiclos, contadorIOCiclos, System.currentTimeMillis(), 0, 0, 0);
          if (estado == EstadoProceso.TERMINADO) {
              this.tiempoFinalizacion = this.tiempoLlegada;
          }
    }
    ...
    public static Proceso fromStringData(String data) throws IllegalArgumentException {
          ...
          try {
              ...
              long tLlegada = (parts.length > 13 && !parts[13].isEmpty()) ? Long.parseLong(parts[13]) : System.currentTimeMillis();
              long tFin = (parts.length > 14 && !parts[14].isEmpty()) ? Long.parseLong(parts[14]) : 0;
              long tBloq = (parts.length > 15 && !parts[15].isEmpty()) ? Long.parseLong(parts[15]) : 0;
              long tListo = (parts.length > 16 && !parts[16].isEmpty()) ? Long.parseLong(parts[16]) : 0;
    ...
    public long getTiempoRetorno() {
        return (tiempoFinalizacion > 0) ? tiempoFinalizacion - tiempoLlegada : -1;
    }

    public long getTiempoRespuesta() {
        long tRetorno = getTiempoRetorno();
        return (tRetorno >= 0) ? tRetorno - tiempoTotalBloqueado : -1;
    }
    ...
    public void setEstado(EstadoProceso nuevoEstado) {
        ...
        if (estadoAnterior != nuevoEstado) {
            if (estadoAnterior == EstadoProceso.LISTO || estadoAnterior == EstadoProceso.SUSPENDIDO_LISTO) {
                tiempoTotalEsperandoListo += (ahora - tiempoInicioRafagaActual);
            }
            else if (estadoAnterior == EstadoProceso.BLOQUEADO || estadoAnterior == EstadoProceso.SUSPENDIDO_BLOQUEADO) {
    ...
    public boolean isSuspendidoGUI() {
       return estado == EstadoProceso.SUSPENDIDO_LISTO || estado == EstadoProceso.SUSPENDIDO_BLOQUEADO;
    }
    ...
    public void mostrarPCB() {
        ...
        if (tipo == TipoBound.I_O_BOUND) {
            if (estado == EstadoProceso.BLOQUEADO || estado == EstadoProceso.SUSPENDIDO_BLOQUEADO) {
                  ioInfo = String.format(" | E/S: %d/%d", contadorIOCiclos, ciclosParaSatisfacerIO);
            } else {
                  ioInfo = String.format(" | Próx E/S en %d ciclos CPU", ciclosParaInterrupcion - contadorCiclos);
            }
        }
